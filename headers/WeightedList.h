...
public:
	...
	struct qsortinfo {
		...
		bool (*cmpfnc)(WeightedNode<T> *, WeightedNode<T> *);
	};

	static unsigned int weightPartition(qsortinfo &inf, unsigned int pivotind) {
		WeightedNode<T> **vals = inf.vals;
		unsigned int beg = inf.beg, end = inf.end;
		auto cmpfnc = inf.cmpfnc;
		WeightedNode<T> *pivot = vals[pivotind];
		...
		//std::cout<<"Partitioned section "<<beg<<" to "<<end<<"\n";
		return storeind;
	}

	static void weightQsort(qsortinfo &inf, unsigned int &maxthreads)
	{
		auto vals = inf.vals;
		unsigned int beg = inf.beg, end = inf.end;
		if(beg >= end) return;
		unsigned int pivotval = end;
		unsigned int piv = weightPartition(inf, pivotval);
		qsortinfo linf, rinf;
		bool ldothread = false;
		bool rdothread = false;
		if(inf.numthreads < maxthreads) {inf.numthreads++; ldothread = true;}
		if(inf.numthreads < maxthreads) {inf.numthreads++; rdothread = true;}
		linf.vals = vals; linf.beg = inf.beg; linf.end = piv-1; linf.cmpfnc = inf.cmpfnc;  linf.numthreads = inf.numthreads;
		rinf.vals = vals; rinf.beg = piv+1; rinf.end = end; rinf.cmpfnc = inf.cmpfnc; rinf.numthreads = inf.numthreads;
		std::thread lthread, rthread;
		if(piv > 0) {
			if(ldothread) lthread = std::thread(weightQsort, linf, maxthreads);
			else weightQsort(linf, maxthreads);
		}
		if(rdothread) rthread = std::thread(weightQsort, rinf, maxthreads);
		else weightQsort(rinf, maxthreads);
		if(lthread.joinable()) lthread.join();
		if(rthread.joinable()) rthread.join();
	}

	void quicksort(unsigned int maxthreads = std::thread::hardware_concurrency()) {
		if(size_ <= 1) return;
		mictimer timer("ms WeightedList::quicksort", 1000.0);
		WeightedNode<T> **nodeArr = new WeightedNode<T>*[size_];
		unsigned int cnt = 0;
		for(WeightedNode<T> *node = first; node != nullptr; node = node->nextnode()) {
			nodeArr[cnt++] = node;
		}
		qsortinfo inf;
		inf.vals = nodeArr;
		inf.beg = 0;
		inf.end = size_-1;
		inf.cmpfnc = WeightedNode<T>::stdcmp;
		inf.numthreads = 0;
		weightQsort(inf, maxthreads);
		first = nodeArr[0];
		last = first;
		prev = first;
		prevind_ = 0;
